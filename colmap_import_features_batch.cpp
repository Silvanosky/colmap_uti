...
int main(int argc, char** argv) {
    ...
    if (OnlyCreateImsInDB) {
        ...
        Camera camera = Camera::CreateFromModelName(1, CamModel, aFocal, aSzX, aSzY);
        ...
        for (auto aIm1 : aImageList) {
            if ((aIm1.find("png") != std::string::npos) ||
                (aIm1.find("tif") != std::string::npos) ||
                (aIm1.find("jpg") != std::string::npos)) {
                aIm1 = aIm1.substr(2);
                auto aIm1Base = std::filesystem::path(aIm1).filename().string() +
                                std::filesystem::path(aIm1).extension().string();

                Image aImCol;
                if (!database.ExistsImageWithName(aIm1Base)) {
                    aImCol.SetName(aIm1Base);
                    aImCol.SetCameraId(camera.camera_id);
    ...
    if (!OnlyCreateImsInDB) {
        ...
        for (auto aIm1 : aImageList) {
            if ((aIm1.find("png") != std::string::npos) ||
                ...
                // std::cout << boost::filesystem::basename(aIm1) << "
                // ewelinaaa" << "\n";
                std::string aPastisDir = aHomName + "/Pastis" + aIm1Base;
                std::vector<std::string> aMatchedFiles =
                    GetFileList(aPastisDir);
                ...
                for (auto aMatchFile : aMatchedFiles) {
                    ...
                    aIm2 = aIm2.substr(0, aIm2.size() - 4);  // remove txt or
                                                             // dat

                    // if this match is part of image_path pattern
                    if (std::find(aImageList.begin(), aImageList.end(),
                                  aImagePath + aIm2) != aImageList.end()) {
                        ...
                        std::vector<std::pair<int, int>>::iterator
                            aFMatchesImIdIter;
                        aFMatchesImIdIter = std::find(
                            aFMatchesImId.begin(), aFMatchesImId.end(),
                            std::pair<int, int>(aIm2Col.ImageId(),
                                                aIm1Col.ImageId()));
                        ...
                        if (aFMatchesImIdIter == aFMatchesImId.end()) {
                            ...
                            // read existing featurepoints  (whether empty or
                            // not)
                            ...
                            std::ifstream aFIn(aMatchFile.c_str(),
                                               std::ifstream::in);
                            while (aFIn.good()) {
                                aFIn >> aKP1.x >> aKP1.y >> aKP2.x >> aKP2.y >>
                                    Pds;

                                // update the global features for the current
                                // pair
                                aFKPts1->push_back(aKP1);
                                aFKPts2->push_back(aKP2);

                                // save the global matches
                                aFMatches.push_back(FeatureMatch(
                                    aFKPts1->size() - 1, aFKPts2->size() - 1));

                                // std::cout << aKP1.x << " " << aKP1.y << " "
                                // << aKP2.x << " " << aKP2.y <<  " " <<
                                // aFKPts1->size()-1 << " " << aFKPts2->size()-1
                                // << "\n";
                            }
                            ...
                            aFMatchesImId.push_back(std::pair<int, int>(
                                aIm1Col.ImageId(), aIm2Col.ImageId()));
                            ...
                            if (Do2ViewGeom) {
                                ...
                                std::cout
                                    << " 2ViewGeom=> " << camera1.camera_id
                                    << " F=" << camera1.FocalLength() << " "
                                    << camera2.camera_id
                                    << " F=" << camera2.FocalLength() << "\n";
                                // getchar();

                                two_view_geometry_options.min_num_inliers = 10;
                                two_view_geometry_options.ransac_options
                                    .max_error = 4;
                                two_view_geometry_options.ransac_options
                                    .confidence = 0.999;
                                two_view_geometry_options.ransac_options
                                ...
                                EstimateCalibratedTwoViewGeometry(
                                    ...
                                    camera2,
                                    FeatureKeypointsToPointsVector(*aFKPts2),
                                    aFMatches, two_view_geometry_options);
        ...
        for (auto aFKs : aFKPtsMap) {
            if (database.ExistsImageWithName(aFKs.first)) {
                ...
                std::cout << "image exists " << aImCol.ImageId()
                          << " saving keypoints \n";
                database.WriteKeypoints(aImCol.ImageId(), *(aFKs.second));
            } else {
                std::cout << "Image " << aFKs.first
        ...
        if (aFMatchesImId.size() == aFMatchesVec.size())
            for (int aK = 0; aK < int(aFMatchesVec.size()); aK++) {
                std::cout << "match " << aFMatchesImId.at(aK).first << " "
                          << aFMatchesImId.at(aK).second
                          << " saving matches \n";
                database.WriteMatches(aFMatchesImId.at(aK).first,
                                      aFMatchesImId.at(aK).second,
                                      aFMatchesVec.at(aK));
            }
    }
