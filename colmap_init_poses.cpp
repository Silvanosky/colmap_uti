...
int main(int argc, char** argv) {
    ...
    for (auto aPName : aPoseList) {
        ...
        if (aExt == std::filesystem::path(aPName).extension().string()) {
            // get image name
            auto aImName = std::filesystem::path(aPName).filename().string() + aExtIm;
            // std::cout << aImageDir+aImName << " " << aImageList[0] << "\n";

            if (std::find(aImageList.begin(), aImageList.end(),
                          aImageDir + aImName) != aImageList.end()) {
                ...
                int aNb = sscanf(aLine, "%lf %lf %lf %lf", &aP(0, 0), &aP(0, 1),
                                 &aP(0, 2), &aP(0, 3));

                // 2nd line
                std::fgets(aLine, 100, pFile);
                aNb = sscanf(aLine, "%lf %lf %lf %lf", &aP(1, 0), &aP(1, 1),
                             &aP(1, 2), &aP(1, 3));

                // 3rd line
                std::fgets(aLine, 100, pFile);
                aNb = sscanf(aLine, "%lf %lf %lf %lf", &aP(2, 0), &aP(2, 1),
                             &aP(2, 2), &aP(2, 3));
                ...
                Eigen::Matrix<double, 3, 3> aMatTmp =
                    (aRotZ * aR * aRotZ).transpose();
                Eigen::Quaterniond Qvec(aMatTmp);
                Eigen::Vector3d Tvec =
                    -(aRotZ * aMatTmp * aRotZ).inverse() *
                    (-aR.inverse() * (aPNorm.col(3) - aOffset));

                // Qvec = RotationMatrixToQuaternion[(RotZ.transpose * RotF *
                // RotZ).transpose ] T = -(RotZ* (RotZ.transpose * RotF *
                // RotZ).transpose  *RotZ).inverse() * - RotF^-1 * TF
                ...
                // std::cout << aImCol.ImageId() << " " << Qvec.transpose() << "
                // " << Tvec.transpose() << " " << aCamId << " " << aImName <<
                // "\n"; getchar();

                // #   IMAGE_ID, QW, QX, QY, QZ, TX, TY, TZ, CAMERA_ID, NAME
                pFileIm << aImCol.ImageId() << " " << Qvec.w() << " " << Qvec.x()
                        << " " << Qvec.y() << " " << Qvec.z() << " " << Tvec(0)
                        << " " << Tvec(1) << " " << Tvec(2) << " " << aCamId
                        << " " << aImName << "\n";
                pFileIm << "\n";
            }
            ...
            /*
                ...
                Qvec = RotationMatrixToQuaternion[(RotZ.transpose * RotF *
RotZ).transpose ] T = -(RotZ* (RotZ.transpose * RotF * RotZ).transpose
*RotZ).inverse() * - RotF^-1 * TF
...
==========================
            ...
            */
            /*
                //camera file
                std::string aPathFile = output_path+"/" + aIm.Name() +
               ".camera"; std::ofstream file(aPathFile, std::ios::trunc);
                CHECK(file.is_open()) << aPathFile;
                file.precision(17);

                Eigen::Matrix3d aR =
               QuaternionToRotationMatrix(aIm.Qvec()).transpose();
                Eigen::Vector3d at = - aR*aIm.Tvec();

                file << aCams[aImCamId].FocalLengthX() << " 0 " <<
               aCams[aImCamId].PrincipalPointX() << "\n"; file << "0 " <<
               aCams[aImCamId].FocalLengthY() << " " <<
               aCams[aImCamId].PrincipalPointY() << "\n"; file << "0 0 1\n0 0
               0\n"; file << aR.row(0) << "\n"; file << aR.row(1) << "\n"; file
               << aR.row(2) << "\n"; file << at(0) << " " << at(1) << " " <<
               at(2) << "\n"; file << aCams[aImCamId].Width() << " " <<
               aCams[aImCamId].Height() << "\n";
